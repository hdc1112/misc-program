...
Well-defined: $1-1+1-1+1-1....$ is not well-defined.
\bigbreak

Let $a_1,a_2,...\in\mathbb{R}$, $\sum\limits_{i=1}^{\infty}a_i=\sum\limits_{i:a_i\ge 0}a_i
+\sum\limits_{i:a_i<0}a_i=b+c$. Then $\sum\limits_{i=1}^{\infty}a_i$ is well-defined if 
either $b$ or $c$ is finite.
\bigbreak

Example: $E(X)$ may exist and could be $\infty$.
...
But $E(X)=\sum\limits_{k=1}^{\infty}\frac{k}{ck^2}=
