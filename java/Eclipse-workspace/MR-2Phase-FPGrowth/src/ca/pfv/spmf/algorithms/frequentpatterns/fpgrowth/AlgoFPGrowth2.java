...
public class AlgoFPGrowth2 {
	 ...
	 */
	private Itemsets runAlgorithm(String input, String output, double minsupp)
	...
	public Itemsets runAlgorithm2(ArrayList<String> dataset, String output,
		...
		scanDatabaseToDetermineFrequencyOfSingleItems2(dataset, mapSupport);
	...
	private void scanDatabaseToDetermineFrequencyOfSingleItems2(
			ArrayList<String> dataset, final Map<Integer, Integer> mapSupport)
			throws FileNotFoundException, IOException {
		// Create object for reading the input file
		// BufferedReader reader = new BufferedReader(new FileReader(input));
		// String line;
	 ...
	 */
	private void fpgrowth(FPTree tree, int[] prefixAlpha, int prefixSupport,
		...
		if (tree.hasMoreThanOnePath == false) {
			...
			addAllCombinationsForPathAndPrefix(tree.root.childs.get(0),
					prefixAlpha); // CORRECT?

		} else { // There is more than one path
			fpgrowthMoreThanOnePath(tree, prefixAlpha, prefixSupport,
					mapSupport);
		}
	}
	 ...
	 *             exception if error writing the output file
	 */
	private void fpgrowthMoreThanOnePath(FPTree tree, int[] prefixAlpha,
	 ...
	 * @throws IOException
	 *             exception if error while writing the output file
	 */
	...
	public void printStats() {
		...
		System.out.println(" Transactions count from database : "
				+ transactionCount);
		System.out.print(" Max memory usage: " + memoryLogger.getMaxMemory()
				+ " mb \n");
